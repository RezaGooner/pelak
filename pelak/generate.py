...
def generate_plate(
    ...
    if not isinstance(plate_code, str):
        raise ValueError("plate-code must be a string.")
    if len(plate_code) < 6 or len(plate_code) > 13:
        raise ValueError("plate-code length is invalid. Expected 6-13 characters.")
    ...
    if not letter_type:
        raise ValueError("Invalid plate-code format or unsupported plate type.")
    ...
    if letter_type == "normal":
        ...
        if extra_month is None or extra_year is None:
            raise ValueError("For 'گ' (gozar) plates, both extra_month and extra-year must be provided.")
    ...
    if len(state_code) > 2:
        raise ValueError("state-code must be 2-digit.")
...
def  generate_plates(
    ...
    out_path   : str    = None,
    show_img   : bool   = False,
    extra_month: list   = None,      # Only for 'گ' plates
    extra_year : list   = None       # Only for 'گ' plates
):
    result =[]
    i = 0
    img = 0
    for plate in plates:
        if plate[5]=='گ':
            img = generate_plate(plate, out_path, show_img,
                                 extra_month[i], extra_year[i])
            i+=1
        else :
            img = generate_plate(plate, out_path, show_img)
        if (show_img):
...
# mapping persian letter to english letter
...
def _get_city_plateinfo_path(txt_file_path="city_plateinfo.txt"):
    if os.path.isabs(txt_file_path):
        return txt_file_path
    data_dir = resource_path('data')
    return os.path.join(data_dir, txt_file_path)

def _read_city_plate_data(txt_file_path="city_plateinfo.txt"):
    file_path = _get_city_plateinfo_path(txt_file_path)
    plates = []
    with open(file_path, encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) != 3:
                continue
            letter, state_number, city = parts
            plates.append({'letter': letter, 'state_number': state_number, 'city': city})
    return plates

def _rand_not_zero(length):
    while True:
        number = ''.join(random.choices("123456789", k=length))
        if '0' not in number:
            return number
# inverse mapping
persian_to_latin_letter = {v:k for k,v in latin_to_persian_letter.items()}

def random_generate_plate(
    city: str = None,
    state_code: str = None,
    letter: str = None,
    txt_file_path: str = "city_plateinfo.txt",
    show_img: bool = False,
    out_path: str = None
):
    all_plates = _read_city_plate_data(txt_file_path)

    selected = None

    if city and state_code and letter:
        # Normalize the letter input: could be either Persian or Latin
        if len(letter) == 1 and ord(letter) >= 0x0600:
            # Persian
            letter_latin = persian_to_latin_letter.get(letter)
            letter_filter = letter_latin if letter_latin else letter.upper()
        else:
            # Latin
            letter_filter = letter.upper()
        filtered = [
            rec for rec in all_plates
            if rec['city'].lower() == city.lower()
            and rec['state_number'] == state_code
            and rec['letter'].upper() == letter_filter
        ]
        if not filtered:
            raise ValueError(f"No plate record found for city '{city}', code '{state_code}', letter '{letter}'")
        selected = random.choice(filtered)
    elif city and letter:
        if len(letter) == 1 and ord(letter) >= 0x0600:
            letter_latin = persian_to_latin_letter.get(letter)
            letter_filter = letter_latin if letter_latin else letter.upper()
        else:
        ...
        if not filtered:
            raise ValueError(f"No plate record found for city '{city}', letter '{letter}'")
        selected = random.choice(filtered)
    elif state_code and letter:
        if len(letter) == 1 and ord(letter) >= 0x0600:
            letter_latin = persian_to_latin_letter.get(letter)
            letter_filter = letter_latin if letter_latin else letter.upper()
        else:
        ...
        filtered = [
            rec for rec in all_plates
            if rec['state_number'] == state_code
            and rec['letter'].upper() == letter_filter
        ]
        if not filtered:
            raise ValueError(f"No plate record found for state code '{state_code}', letter '{letter}'")
        selected = random.choice(filtered)
        ...
        if not filtered:
            raise ValueError(f"No plate record found for city '{city}' with state code '{state_code}'")
        selected = random.choice(filtered)
    elif city:
        filtered = [
            rec for rec in all_plates
        ...
        if not filtered:
            raise ValueError(f"No plate record found for city '{city}'")
        selected = random.choice(filtered)
    elif state_code:
        filtered = [
            rec for rec in all_plates
        ...
        if not filtered:
            raise ValueError(f"No plate record found for state-code '{state_code}'")
        selected = random.choice(filtered)
    else:
        raise ValueError("At least one of city or state-code must be provided.")

    # Mapping letter_latin to Persian for plate code
    letter_latin = selected["letter"].upper()
    letter_persian = latin_to_persian_letter.get(letter_latin)
    if not letter_persian:
        raise ValueError(f"Cannot map latin plate letter '{letter_latin}' to Persian.")

    left2  = _rand_not_zero(2)
    right3 = _rand_not_zero(3)
    code   = selected["state_number"]
    plate_code = f"{left2}{letter_persian}{right3}{code}"

    img = generate_plate(plate_code, out_path=out_path, show_img=show_img)
    return img
